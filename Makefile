...
repo:
	...
	kubectl --namespace gitea port-forward svc/gitea-http 3000:3000  > /dev/null 2>&1 & curl -v -s -XPOST -H "Content-Type: application/json" -k -u 'gitea_admin:r8sA8CPHD9!bt6d' http://localhost:3000/api/v1/admin/unadopted/gitea_admin/local-repo &&  curl -v -s -XPATCH -H "Content-Type: application/json" -k -d '{"private": false}' -u 'gitea_admin:r8sA8CPHD9!bt6d' http://localhost:3000/api/v1/repos/gitea_admin/local-repo	
